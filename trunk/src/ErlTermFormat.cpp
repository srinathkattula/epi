...
ErlTerm * epi::type::ErlTerm::format( const char* formatStr, ... )
        throw (EpiParseError)  
{
    ErlTerm *res=0;
    va_list ap;
    ...
    va_start(ap, formatStr);
    char** fmt = const_cast<char**>(&formatStr);
    res = epi::parse::eformat(fmt, &ap);
    va_end(ap);

    if (res == 0) {
        throw EpiParseError("Parsing error.", *fmt - formatStr);
    }
