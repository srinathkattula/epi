...
public:
    ...
    template<int N>
    ErlConsList(ErlTerm* (&elems)[N]) throw(EpiBadArgument) {
        new(*this) ErlConsList(elems, N);
    }
    ...
    template<int N>
    void init(ErlTerm* (&elems)[N]) { init(elems, N); }
    ...
    ErlTerm* operator[] (size_t i) const { return elementAt(i); }

    ErlTerm* tail(unsigned int index) const
            throw(EpiInvalidTerm, EpiBadArgument, EpiEmptyList);
