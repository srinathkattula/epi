...
int main (int argc, char **args) {
    ...
    */
    ...
    try {
        ...
        Connection* connection = node.connect(REMOTENODE, COOKIE);

        if (!connection)
            throw EpiException("Can't create connection to node!");
            
        // Get a mailbox. The node has the pointer owership!!!
        MailBox *mailbox = node.createMailBox(connection);

        if (!mailbox)
            throw EpiException("Can't create mailbox!");
            
		// Create {self(), hello} tuple
        ErlTerm* terms[] = {mailbox->self(), new ErlAtom("hello")};
        ErlTermPtr<ErlTuple> tuple(new ErlTuple(terms));
