...
public:
     ...
     **/
    inline ErlLong(const long long value): mValue(value) { mInitialized=true; }
     ...
     */
    void init(const long long value) throw(EpiAlreadyInitialized);

    /**
     * Get the actual value contained in this term.
     **/
    long long longValue() const throw(EpiInvalidTerm) {
        if (!isValid()) throw EpiInvalidTerm("Double is not initialized");
        return mValue;
    }

    bool equals(const ErlTerm &t) const {
        if (!t.instanceOf(ERL_LONG) || !isValid() || !t.isValid())
            return false;
        else 
            return mValue == reinterpret_cast<const ErlLong&>(t).mValue;
    }

    bool operator== (const ErlTerm& t) const { return this->equals(t); }
